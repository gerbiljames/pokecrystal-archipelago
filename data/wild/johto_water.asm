; Johto Pokémon in water

JohtoWaterWildMons:

	def_water_wildmons RUINS_OF_ALPH_OUTSIDE
.AP_WildWater_RUINS_OF_ALPH_OUTSIDE
	db 2 percent ; encounter rate
	db 15, WOOPER
	db 20, QUAGSIRE
	db 15, QUAGSIRE
	end_water_wildmons

	def_water_wildmons UNION_CAVE_1F
.AP_WildWater_UNION_CAVE_1F
	db 2 percent ; encounter rate
	db 15, WOOPER
	db 20, QUAGSIRE
	db 15, QUAGSIRE
	end_water_wildmons

	def_water_wildmons UNION_CAVE_B1F
.AP_WildWater_UNION_CAVE_B1F
	db 2 percent ; encounter rate
	db 15, WOOPER
	db 20, QUAGSIRE
	db 15, QUAGSIRE
	end_water_wildmons

	def_water_wildmons UNION_CAVE_B2F
.AP_WildWater_UNION_CAVE_B2F
	db 4 percent ; encounter rate
	db 15, TENTACOOL
	db 20, QUAGSIRE
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons SLOWPOKE_WELL_B1F
.AP_WildWater_SLOWPOKE_WELL_B1F
	db 2 percent ; encounter rate
	db 15, SLOWPOKE
	db 20, SLOWPOKE
	db 10, SLOWPOKE
	end_water_wildmons

	def_water_wildmons SLOWPOKE_WELL_B2F
.AP_WildWater_SLOWPOKE_WELL_B2F
	db 2 percent ; encounter rate
	db 15, SLOWPOKE
	db 20, SLOWPOKE
	db 20, SLOWBRO
	end_water_wildmons

	def_water_wildmons ILEX_FOREST
.AP_WildWater_ILEX_FOREST
	db 2 percent ; encounter rate
	db 15, PSYDUCK
	db 10, PSYDUCK
	db 15, GOLDUCK
	end_water_wildmons

	def_water_wildmons MOUNT_MORTAR_1F_OUTSIDE
.AP_WildWater_MOUNT_MORTAR_1F_OUTSIDE
	db 4 percent ; encounter rate
	db 15, GOLDEEN
	db 20, MARILL
	db 20, SEAKING
	end_water_wildmons

	def_water_wildmons MOUNT_MORTAR_2F_INSIDE
.AP_WildWater_MOUNT_MORTAR_2F_INSIDE
	db 2 percent ; encounter rate
	db 20, GOLDEEN
	db 25, MARILL
	db 25, SEAKING
	end_water_wildmons

	def_water_wildmons MOUNT_MORTAR_B1F
.AP_WildWater_MOUNT_MORTAR_B1F
	db 2 percent ; encounter rate
	db 15, GOLDEEN
	db 20, MARILL
	db 20, SEAKING
	end_water_wildmons

	def_water_wildmons WHIRL_ISLAND_SW
.AP_WildWater_WHIRL_ISLAND_SW
	db 4 percent ; encounter rate
	db 20, TENTACOOL
	db 15, HORSEA
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons WHIRL_ISLAND_B2F
.AP_WildWater_WHIRL_ISLAND_B2F
	db 4 percent ; encounter rate
	db 15, HORSEA
	db 20, HORSEA
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons WHIRL_ISLAND_LUGIA_CHAMBER
.AP_WildWater_WHIRL_ISLAND_LUGIA_CHAMBER
	db 4 percent ; encounter rate
	db 20, HORSEA
	db 20, TENTACRUEL
	db 20, SEADRA
	end_water_wildmons

	def_water_wildmons SILVER_CAVE_ROOM_2
.AP_WildWater_SILVER_CAVE_ROOM_2
	db 2 percent ; encounter rate
	db 35, SEAKING
	db 35, GOLDUCK
	db 35, GOLDEEN
	end_water_wildmons

	def_water_wildmons DARK_CAVE_VIOLET_ENTRANCE
.AP_WildWater_DARK_CAVE_VIOLET_ENTRANCE
	db 2 percent ; encounter rate
	db 15, MAGIKARP
	db 10, MAGIKARP
	db 5, MAGIKARP
	end_water_wildmons

	def_water_wildmons DARK_CAVE_BLACKTHORN_ENTRANCE
.AP_WildWater_DARK_CAVE_BLACKTHORN_ENTRANCE
	db 2 percent ; encounter rate
	db 15, MAGIKARP
	db 10, MAGIKARP
	db 5, MAGIKARP
	end_water_wildmons

	def_water_wildmons DRAGONS_DEN_B1F
.AP_WildWater_DRAGONS_DEN_B1F
	db 4 percent ; encounter rate
	db 15, MAGIKARP
	db 10, MAGIKARP
	db 10, DRATINI
	end_water_wildmons

	def_water_wildmons OLIVINE_PORT
.AP_WildWater_OLIVINE_PORT
	db 2 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons ROUTE_30
.AP_WildWater_ROUTE_30
	db 2 percent ; encounter rate
	db 20, POLIWAG
	db 15, POLIWAG
	db 20, POLIWHIRL
	end_water_wildmons

	def_water_wildmons ROUTE_31
.AP_WildWater_ROUTE_31
	db 2 percent ; encounter rate
	db 20, POLIWAG
	db 15, POLIWAG
	db 20, POLIWHIRL
	end_water_wildmons

	def_water_wildmons ROUTE_32
.AP_WildWater_ROUTE_32
	db 6 percent ; encounter rate
	db 15, TENTACOOL
	db 20, QUAGSIRE
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons ROUTE_34
.AP_WildWater_ROUTE_34
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons ROUTE_35
.AP_WildWater_ROUTE_35
	db 4 percent ; encounter rate
	db 20, PSYDUCK
	db 15, PSYDUCK
	db 20, GOLDUCK
	end_water_wildmons

	def_water_wildmons ROUTE_40
.AP_WildWater_ROUTE_40
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons ROUTE_41
.AP_WildWater_ROUTE_41
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 20, TENTACRUEL
	db 20, MANTINE
	end_water_wildmons

	def_water_wildmons ROUTE_42
.AP_WildWater_ROUTE_42
	db 4 percent ; encounter rate
	db 20, GOLDEEN
	db 15, GOLDEEN
	db 20, SEAKING
	end_water_wildmons

	def_water_wildmons ROUTE_43
.AP_WildWater_ROUTE_43
	db 2 percent ; encounter rate
	db 20, MAGIKARP
	db 15, MAGIKARP
	db 10, MAGIKARP
	end_water_wildmons

	def_water_wildmons ROUTE_44
.AP_WildWater_ROUTE_44
	db 2 percent ; encounter rate
	db 25, POLIWAG
	db 20, POLIWAG
	db 25, POLIWHIRL
	end_water_wildmons

	def_water_wildmons ROUTE_45
.AP_WildWater_ROUTE_45
	db 2 percent ; encounter rate
	db 20, MAGIKARP
	db 15, MAGIKARP
	db 5, MAGIKARP
	end_water_wildmons

	def_water_wildmons NEW_BARK_TOWN
.AP_WildWater_NEW_BARK_TOWN
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons CHERRYGROVE_CITY
.AP_WildWater_CHERRYGROVE_CITY
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons VIOLET_CITY
.AP_WildWater_VIOLET_CITY
	db 2 percent ; encounter rate
	db 20, POLIWAG
	db 15, POLIWAG
	db 20, POLIWHIRL
	end_water_wildmons

	def_water_wildmons CIANWOOD_CITY
.AP_WildWater_CIANWOOD_CITY
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons OLIVINE_CITY
.AP_WildWater_OLIVINE_CITY
	db 6 percent ; encounter rate
	db 20, TENTACOOL
	db 15, TENTACOOL
	db 20, TENTACRUEL
	end_water_wildmons

	def_water_wildmons ECRUTEAK_CITY
.AP_WildWater_ECRUTEAK_CITY
	db 2 percent ; encounter rate
	db 20, POLIWAG
	db 15, POLIWAG
	db 20, POLIWHIRL
	end_water_wildmons

	def_water_wildmons LAKE_OF_RAGE
.AP_WildWater_LAKE_OF_RAGE
	db 6 percent ; encounter rate
	db 15, MAGIKARP
	db 10, MAGIKARP
	db 15, GYARADOS
	end_water_wildmons

	def_water_wildmons BLACKTHORN_CITY
.AP_WildWater_BLACKTHORN_CITY
	db 4 percent ; encounter rate
	db 15, MAGIKARP
	db 10, MAGIKARP
	db 5, MAGIKARP
	end_water_wildmons

	def_water_wildmons SILVER_CAVE_OUTSIDE
.AP_WildWater_SILVER_CAVE_OUTSIDE
	db 2 percent ; encounter rate
	db 35, POLIWHIRL
	db 40, POLIWHIRL
	db 35, POLIWAG
	end_water_wildmons

	db -1 ; end
